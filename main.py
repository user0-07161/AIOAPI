...
class API(BaseHTTPRequestHandler):
    ...
    def handle_rankcard(self, query_components):
        ...
        font = ImageFont.truetype("Roboto-BoldItalic.ttf", 40)
        x=10
        y=150
        progress=50
        bg=(129, 66, 97)
        ...
        try:
           if nextlevelxp and currentxp:
                draw.text((30, 100), f"{currentxp}/{nextlevelxp} XP", font = font, align ="left")
        except:
           pass
        buffer = BytesIO()
    ...
    def handle_amongus(self, query_components):
        img = Image.open("Background-Amongus.png")
        draw = ImageDraw.Draw(img)
        for key, values in query_components.items():
            if key == "impostor":
                if values[0] == "true":
        ...
        try:
           if crewmate:
             ...
             crewmateimg = Image.open(f"{crewmate}.png")
             print(f"Crewmate: {crewmate}.png")
             crewmateimg.thumbnail((100,100))
             rotatedcrewmateimg = crewmateimg.rotate(random.randint(1,361)).convert("RGBA")
